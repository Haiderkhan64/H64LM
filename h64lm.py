...
Original file is located at
    https://colab.research.google.com/drive/1mlRCkqEct6FpX-uSOmiOQC9MnmeMoZos
...
class H64LMConfig:
    ...
    def __post_init__(self):
        ...
        if self.expert_hidden_size is None:
            self.expert_hidden_size = self.hidden_size * 4
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_path = os.path.join(checkpoint_dir, "best_model_state_dict.pt")
...
def main():
    ...
    # Load dataset
    logger.info(f">>> Loading dataset: {training_config.dataset} /{training_config.dataset_dir}")
    ...
    tokenizer_name = training_config.use_pretrained_tokenizer
    ...
    # Multi-GPU support
    if training_config.use_ddp and torch.cuda.device_count() > 1:
    ...
    total_steps = (len(train_loader_temp) * training_config.num_epochs ) // training_config.grad_accum_steps
    base_lr = 3e-4
    no_decay = ["bias", "LayerNorm.weight", "layernorm.weight", "norm.weight"]
    ...
    if training_config.resume_from and os.path.exists(training_config.resume_from):
        print(f"[INFO] >>> Attempting to resume from: {training_config.resume_from}")
        start_epoch, _ = load_checkpoint_for_resume(
            model, optimizer, scheduler, training_config.resume_from , device
        )

        # If resuming, adjust remaining epochs
        if start_epoch > 0:
            training_config.num_epochs = max(training_config.num_epochs, start_epoch + 10)  # Train 10 more epochs
            print(f"[INFO] >>> Will train until epoch {training_config.num_epochs}")
